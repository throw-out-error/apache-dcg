...
# Domain setup
domainAnswer = input(
    "What are your domain names? [example.com] \n> ").lower() or "example.com"
...
if initPrm == "proxy":
    portForForward = input(
        "Please enter the port you wish to forward [8080] \n> ") or "8080"
    apcPa = input(
        "Where is your Apache installation located? [/etc/apache2/] \n> ") or "/etc/apache2"
    proxyConf()
elif initPrm == "webroot":
    webLoc = input(
        "What is the absolute path of the files for your website?\n> ") or "/var/www/html"
    apcPa = input(
        "Where is your Apache installation located? [/etc/apache2/] \n> ") or "/etc/apache2"
    regConf()
else:
...
restart = input(
    "Would you like to reload the configuration now? [n] \n> ").lower() or "n"
